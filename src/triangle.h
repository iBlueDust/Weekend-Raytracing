...
public:
	...
	virtual std::optional<BoundingBox> boundingBox(
		...
		return BoundingBox(
			point3(
				std::min<double>({ a.x, b.x, c.x }),
				std::min<double>({ a.y, b.y, c.y }),
				std::min<double>({ a.z, b.z, c.z })
			),
			point3(
				std::max<double>({ a.x, b.x, c.x }),
				std::max<double>({ a.y, b.y, c.y }),
				std::max<double>({ a.z, b.z, c.z })
			)
		);
	}
